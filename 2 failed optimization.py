...
new_points_x = np.random.randint(min_w, max_w, num_points)
new_points_y = np.random.randint(min_h, max_h, num_points)
...
for num_generations_i in range(num_generations):
    # max_h = 0
    # min_h = 1000000
    # max_w = 0
    # min_w = 1000000
    ...
    buffer = 10
    new_points_x = np.random.randint(min_w - buffer, max_w + buffer, num_points)
    new_points_y = np.random.randint(min_h - buffer, max_h + buffer, num_points)
    new_points = np.column_stack([new_points_y, new_points_x])
    ...
    for num_move_steps_i in range(num_move_steps):
        ...
        cv2.imwrite('./out/' + str(counter) + ".jpg", pixels)
        counter += 1
